...
#define     SIH_SCALE_TO_FIT            0¡Á00000002
	...
	int createD3d10()
	{
		...
		//scd.BufferDesc.RefreshRate.Denominator = 1;            //need 120hz for 3d vision
		...
		if (FAILED(r))
		{
			std::cout << "full screen state error error" << std::endl;
			return r;
		}
	...
	int createStereo()
	{
		...
		m_pD3D10Dev->CreateTexture2D(&desc, &sysData, &m_pStereoParSurface);
	...
	int renderGlass()
	{
		try
		{
			...
			m_pD3D10Dev->CopySubresourceRegion(m_pStereoParSurface, 0, 0, 0, 0, m_pStereoSurface,0, &stereoDataBox);
			...
			m_pD3D10Dev->CopySubresourceRegion(m_pBackBuffer, 0, 0, 0, 0, m_pStereoParSurface, 0, &stereoSrcBox);
	...
	ID3D10Texture2D*        m_pStereoParSurface;
